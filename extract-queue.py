...
from extract_status import (  # noqa: E402
    DEFAULT_DOWNLOADS,
    DEFAULT_VAULT,
    append_recovered,
    artifacts,
...
SCRIPTS = {
    "reel": [str(SCRIPT_DIR / "transcribe-reel.sh")],
    "carousel": [str(SCRIPT_DIR / "transcribe-carousel.sh")],
    "youtube": [str(SCRIPT_DIR / "transcribe-youtube.sh")],
    "twitter": [str(SCRIPT_DIR / "transcribe-twitter.sh")],
}
    ...
    *,
    ...
    try:
        ...
        if ok:
            ...
            # Script died (mjpeg, no-audio, old carousel --print id) but media landed.
            status = "ok_partial"
        else:
            status = "fail"
        row = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--jsonl",
        type=Path,
        default=Path("/tmp/extract.jsonl"),
        help="Audit log (append). Default: /tmp/extract.jsonl",
    )
    ...
    from extract_status import load_jsonl
